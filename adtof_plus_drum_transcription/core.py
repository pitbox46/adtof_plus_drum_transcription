...
def map_energy_to_velocity(
    energy, max_db, min_velocity=0, max_velocity=127, use_log=False
):
    ...
    return np.interp(normalized_energy, [0, 1], [min_velocity, max_velocity]).astype(
        int
    )


def get_loudness(path, threshold=-float("inf"), activity_threshold=None):
    """Extract loudness values from audio file using librosa."""
    ...
    # Convert to dB
    loudness_values = librosa.amplitude_to_db(rms, ref=np.max)

    max_db = np.max(loudness_values)

    if threshold is not None:
        loudness_values = np.where(loudness_values < threshold, -90, loudness_values)

    if activity_threshold is not None:
        if not np.any(loudness_values > activity_threshold):
            return -90, [-90 for _ in loudness_values]

    return max_db, loudness_values.tolist()
...
def nearest_onset_time(onsets, onset_time, use_shift=False, threshold=float("-inf")):
    ...
    if onsets[onset_idx] > -55 and is_monotonic_neighbour(onsets, onset_idx, neighbour):
        ...
        if onsets[onset_idx - 1] > onsets[onset_idx + 1]:
            shift = (
                (onsets[onset_idx + 1] - onsets[onset_idx - 1])
                / (onsets[onset_idx] - onsets[onset_idx + 1])
                / 2
            )
        else:
            shift = (
...
def transcribe_drums(
    audio_path, output_path, input_is_mix=True, default_threshold=-float("inf")
):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for label, path in tqdm.tqdm(
            [
                ("kick", kick_path),
                ("snare", snare_path),
                ("hihat", hihat_path),
                ("cymbals", cymbals_path),
                ("toms", toms_path),
            ],
            desc="Transcribing MIDI from stems",
        ):
            ...
            velocities = map_energy_to_velocity(
                loudness_values, max_db, use_log=use_log
            )
            ...
            for n in mid.instruments[0].notes:
                if n.pitch in [35, 38, 42, 49]:  # kick, snare and hh
                    ...
                    onset_idx, onset_time = nearest_onset_time(
                        loudness_values, n.start, threshold=default_threshold
                    )
                    if onset_idx is not None:
                        ...
                        n.velocity = max(
                            velocities[
                                max(0, onset_idx - 1) : min(
                                    len(velocities), onset_idx + 2
                                )
                            ]
                        )
        ...
        for i in range(len(hihat_notes)):
            if i < len(hihat_notes) - 1:
                ...
                interval_loudness = hihat_loudness[
                    int(interval_start * 100) : int(interval_end * 100)
                ]
        ...
        if cymbals_loudness is not None:
            cymbals_notes = [
                n for n in combined_mid.instruments[0].notes if n.pitch == 49
            ]
            for i in range(len(cymbals_notes)):
                if i < len(cymbals_notes) - 1:
                    interval_start = cymbals_notes[i].start
                    interval_end = min(
                        interval_start + 0.15, cymbals_notes[i + 1].start
                    )

                    interval_loudness = cymbals_loudness[
                        int(interval_start * 100) : int(interval_end * 100)
                    ]
                    if len(interval_loudness) == 0:
                        continue
